...
const TEMPLATE = `
  <table data-part="table-root" id="${rootId}" class="usa-table">
...
it('sorts rows by cell content numerically when clicked', async () => {
  ...
  const tbody = root.querySelector('tbody')!
  const sortableHeaders = root.querySelectorAll('th[data-sortable]')
  const numericSortButton = sortableHeaders[1].querySelector('[data-part="table-sort-button"]')!
...
it('sorts rows by "data-sort-value" attribute on cells when clicked', async () => {
  ...
  const tbody = root.querySelector('tbody')!
  const sortableHeaders = root.querySelectorAll('th[data-sortable]')
  const dataSortValueSortButton = sortableHeaders[2].querySelector('[data-part="table-sort-button"]')!
