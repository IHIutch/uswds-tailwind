...
description: "Getting started with USWDS + Tailwind is fast and easy. Following the guidance below, you'll be up and running in just a few minutes."
---
...
```bash
npm i alpinejs @alpinejs/anchor @alpinejs/focus @alpinejs/mask
```
...
Then, add the `@tailwind` directives for each of Tailwind's layers to your `src/assets/css/styles.css` file.
...
Now, update your `src/assets/js/main.js` file to import your CSS, fonts, and `Alpine`:
...
<html lang="en">
  ...
  <body class="font-source-sans">
    <div class="max-w-prose py-4 px-3 mx-auto">
      <h1 class="text-4xl font-bold">Welcome to USWDS + Tailwind!</h1>
...
Before we push our code to our repository, we'll want to add a `.gitignore` file. This way, we dont push unnecessary or redundant code to our remote repository.
...
**Note:** If you're manually compiling your CSS and JavaScript assets locally before a deployment, don't add the `build` directory to your `.gitignore`.
</ProcessStep>
</ProcessList>
